...
SQL_AGENT_PROMPT = """
                ...
                - When the user’s query involves **searching**, **finding**, or **matching** by project, domain, or technology name — especially multi-word or near-miss text such as “e commerce” vs “e-commerce” — you must use one of the following strategies:
                    1. Prefer `similarity(column, 'query') > 0.5` or the trigram operator `column % 'query'`.
                    2. Combine with `ILIKE` for broader recall:
                    ```
                    where column ILIKE '%e commerce%' 
                        or similarity(column, 'e commerce') > 0.5
                    ```
                ...
                4.  **Complex Name Matching:**
                    ...
                    *   Additionally, you may enhance this with fuzzy pg_trgm matching:
                        ```
                        or similarity(p.name, 'project athena final') > 0.5
                        ```
                ...
                *   **User Query:** "find projects similar to e commerce"  
                    *   **SQL:**  
                        ...
                        or similarity(p.name, 'e commerce') > 0.5 
                        order by similarity(p.name, 'e commerce') desc 
                        limit 10;
                *   **User Query:** "find projects and their document contents related to a given search phrase"  
                    *   **SQL (Generalized Template):**
                        ```sql
                        select distinct
                            p.name,
                            doc.content,
                            similarity(p.name, '<search_phrase>') as name_similarity,
                            similarity(doc.content, '<search_phrase>') as content_similarity,
                            greatest(
                                coalesce(similarity(p.name, '<search_phrase>'), 0),
                                coalesce(similarity(doc.content, '<search_phrase>'), 0),
                                coalesce(similarity(d.name, '<search_phrase>'), 0)
                            ) as max_similarity
                        from project p
                        left join project_domain pd on p.id = pd.project_id
                        left join domain d on pd.domain_id = d.id
                        left join document doc on p.id = doc.project_id
                        where
                            (
                                (
                                    -- match each word in the phrase within project name
                                    (p.name ilike all (array[
                                        % for word in words_of('<search_phrase>') %
                                        concat('%', '<word>', '%')
                                        % endfor %
                                    ]))
                                    or similarity(p.name, '<search_phrase>') > 0.5
                                )
                                or
                                (
                                    -- match within document content
                                    (doc.content ilike all (array[
                                        % for word in words_of('<search_phrase>') %
                                        concat('%', '<word>', '%')
                                        % endfor %
                                    ]))
                                    or similarity(doc.content, '<search_phrase>') > 0.5
                                )
                                or
                                (
                                    -- match within domain name
                                    (d.name ilike all (array[
                                        % for word in words_of('<search_phrase>') %
                                        concat('%', '<word>', '%')
                                        % endfor %
                                    ]))
                                    or similarity(d.name, '<search_phrase>') > 0.5
                                )
                            )
