...
def _retrive_message(user_id, thread_id, role, content, file_name=None, file_path=None):
    ...
    with _user_locks[user_id]:
        ...
        # Retrieve last N messages from DB (say last 5)
        messages = db_manager._retrieve_messages(thread_id, limit=5)
...
def handle_large_tool_output(tool_name: str, content: str, max_chars: int = 16500) -> str:
    """Handle large tool outputs safely (only for execute_sql)."""
    if not content:
        return content

    # Apply truncation only for execute_sql results
    if tool_name == "execute_sql" and len(content) > max_chars:
        truncated = content[:max_chars]
        remainder = len(content) - max_chars
        return (
            truncated
            + f"\n\n-- [Truncated {remainder} characters of SQL result to prevent token overflow]"
        )
...
def _stream_and_save_response(ai_instance, user_id, thread_id, messages) -> tuple[str, Optional[str], Optional[str]]:
    ...
    for step in ai_instance.agent.stream({"messages": messages}, stream_mode="values"):
        ...
        for msg in step.get("messages", []):
            # Intercept and handle large execute_sql output
            if type(msg).__name__ == "ToolMessage" and msg.name == "execute_sql":
                safe_content = handle_large_tool_output(msg.name, msg.content)
                msg.content = safe_content
                logger.info(f"✅ Truncated execute_sql output, new length={len(msg.content)}")
