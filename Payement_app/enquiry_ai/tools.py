...
class AgentTools:
    ...
    def execute_sql_query(self, sql: str, reasoning: str) -> str:
        ...
        try:
            ...
            logger.warning(f"SQL error: {e}. Attempting to fix syntax...")

            fixed_sql = self.fix_sql_syntax(sql)
            if not fixed_sql:
                logger.error(f"Could not fix SQL: {sql}")
            ...
            try:
                ...
                logger.error(f"Fixed SQL also failed: {e2}")
                return json.dumps({"error": f"Original: {e}; Fixed: {e2}"}, indent=2)
            
    #Fix SQL syntax using LLM

    def fix_sql_syntax(self, sql: str) -> Optional[str]:
        """Fix SQL syntax issues using LLM."""
        try:
            prompt = f"""
            Correct the following PostgreSQL query if it has syntax errors.
            ...
            """
            response = self.model.invoke([
                SystemMessage(content="You are an expert in PostgreSQL syntax correction."),
                HumanMessage(content=prompt)
            ])
            fixed_sql = response.content.strip().replace("```sql", "").replace("```", "").strip()

            # Optional sanity check
            if not re.match(r"^(SELECT|INSERT|UPDATE|DELETE|CREATE|DROP|ALTER)", fixed_sql, re.IGNORECASE):
                logger.warning(f"Suspicious fixed SQL output: {fixed_sql}")
                return None
    ...
    def get_project_file(self, project_name: str) -> str:
        ...
        logger.info("Executing get project file tool")
        if not q:
